...
| `eu_all_plants` | **plant_uid**, district, hospital, capacity, **manufacturer**, latest_status, latest_purity, latest_hours, latest_date, eu_leakage, eu_fire_safety, drill_count, **latest_not_running** | `EU_ALL_PLANTS` | **`plant_uid`** = `QR\|district\|hospital\|capacity\|manufacturer` (`euUID`; `c:null`) |
...
| `eu_timeline` | code, **plant_uid**, district, facility, capacity, **manufacturer**, date, purity, status, **eq_status** | `TL2.history` | **keyed by `plant_uid`** (entry keeps `code`) |
